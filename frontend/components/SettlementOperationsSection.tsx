      ...
      <>
        <Card padding="md" className="space-y-3">
          ...
          <p className="text-sm text-gray-600">
            Połącz tę operację z innymi, aby śledzić wspólne rozliczenie (np. wydatek
            i zwroty).
          </p>
        ...
        <SettlementGroupPickerModal
          ...
          title="Wybierz grupę"
          onSelect={(gid) => addToGroupMutation.mutate(gid)}
        />
      </>
    );
  }
  ...
  return (
    <>
      <Card padding="md" className="space-y-3">
        <div className="flex items-start justify-between gap-2">
          <div>
            <SectionLabel>Powiązane operacje</SectionLabel>
            {group.title && (
              <p className="text-sm font-medium text-gray-900 mt-1">{group.title}</p>
            )}
            {group.note && <p className="text-sm text-gray-600 mt-0.5">{group.note}</p>}
          </div>
          <Link
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-sm text-gray-800">
          <div>
            <div className="text-[11px] text-gray-500 uppercase">Suma wydatków (orientacyjnie)</div>
        ...
        {group.linked_receipts.length > 0 && (
          <div>
            <div className="text-[11px] text-gray-500 uppercase tracking-wide mb-1">Paragony w zestawie</div>
        ...
        {others.length > 0 && (
          <div>
            ...
            <ul className="space-y-1.5">
              {others.map((m) => (
                <li key={`${m.source_type}-${m.id}`} className="flex flex-wrap items-baseline justify-between gap-2 text-sm text-gray-800">
                  <span>
                  ...
                  <span className="text-gray-600 whitespace-nowrap text-xs">
                    {isoToDisplay(m.booking_date)} · {formatAmount(m.amount, m.currency || "PLN")}
                  </span>
                </li>
              ))}
            </ul>
      ...
      <ConfirmDeleteModal
        ...
        onConfirm={() => {
          void deleteGroupMutation.mutateAsync(group.id).then(() => setDeleteGroupOpen(false));
        }}
