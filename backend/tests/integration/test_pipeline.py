...
def test_confirm_receipt_persists_transaction(integration_app, migrated_db):
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("INSERT INTO categories (name) VALUES ('Food') RETURNING id")
            category_id = cur.fetchone()[0]
        conn.close()

        from src.data import ConfirmReceiptRequest
        request = ConfirmReceiptRequest(product_categories={"Mleko": category_id})
