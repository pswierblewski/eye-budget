...
class GroundTruthService:
    ...
    def create(self, filename: str, file_data: bytes) -> GroundTruthResponse:
        ...
        try:
            ...
            if entry is None:
                raise Exception("Failed to retrieve created ground truth entry")
    ...
    def create_from_confirmed_receipt(
        ...
        """
        Persist a confirmed receipt as a ground truth entry.

        Skips silently if a ground truth entry for this filename already exists
        or if no MinIO object key is available (image not yet uploaded).
        No OCR re-run is performed — the supplied TransactionModel is stored as-is.
        """
        if minio_object_key is None:
            print(f"[ground-truth] Skipping '{filename}': no minio_object_key available.")
            return

        if self.ground_truth_repository.get_by_filename(filename) is not None:
            print(f"[ground-truth] Skipping '{filename}': entry already exists.")
            return
        ...
        if entry_id == -1:
            print(f"[ground-truth] Failed to create entry for '{filename}'.")
        else:
