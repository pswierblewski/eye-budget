...
from ..data import VendorMapping, VendorItem
...
class VendorsRepository(ABC):
    ...
    def get_normalized_name_by_alternative_name(self, alternative_name: str) -> Optional[str]:
        """
        Return the normalized vendor name that a raw receipt name maps to.

        Args:
            alternative_name: The vendor name as it appears on the receipt

        Returns:
            Normalized vendor name if a mapping exists, None otherwise
        """
        if not self.conn:
            return None
        try:
            with self.conn.cursor() as cursor:
                cursor.execute(
                    """
                    SELECT v.name
                    FROM vendors_alternative_names van
                    JOIN vendors v ON v.id = van.vendor
                    WHERE van.name = %s
                    """,
                    (alternative_name,),
                )
            ...
            print(f"Failed to get normalized vendor name: {e}")
            return None
    ...
    def get_all_vendors(self) -> list["VendorItem"]:
        """
        Return all vendors ordered alphabetically by name.
        ...
        try:
            with self.conn.cursor() as cursor:
                ...
                return [VendorItem(id=row[0], name=row[1]) for row in rows]
        except Exception as e:
